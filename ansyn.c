...
void lr1() {
    ...
    while(1) {
        ...
        if(item == 2147483647) {
            ...
            switch(tla().no) {
            case C_CHAR:
                strcpy(given, "constchar");
                break;
            case C_STRING:
                strcpy(given, "string");
                break;
            case C_NUM:
                strcpy(given, "constnum");
                break;
            case IDENT:
                strcpy(given, "identifier");
                break;
            default:
                sprintf(given, "'%s'", tla().val);
            }
            ...
            for(i = 0; i <= NAL; i++) {
                ...
                if (sel >= 0) {
                    ...
                    switch(i) {
                    case C_CHAR:
                        strcat(expected, "constchar");
                        break;
                    case C_STRING:
                        strcat(expected, "string");
                        break;
                    case C_NUM:
                        strcat(expected, "constnum");
                        break;
                    case IDENT:
                        strcat(expected, "identifier");
                        break;
                    ...
                    case K_VOID:
                    case typedef_name:
                        if(!typed) strcat(expected, "typename");
                        typed = 1;
                        break;
                    ...
                    case K_CONST:
                    case K_EXTERN:
                        if(!speced) strcat(expected, "typespec");
                        speced = 1;
                        break;
                    ...
                    case K_ENUM:
                    case K_UNION:
                        if(!aggred) strcat(expected, "aggregator");
                        aggred = 1;
                        break;
